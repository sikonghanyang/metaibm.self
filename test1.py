...
##function of generating empty mainland##
...
##function of generating empty metacommunity##
...
'''def mkdir_if_not_exist(reproduce_mode, patch_num, is_heterogeneity, disp_among_within_rate, patch_dist_rate):
    ...
    if os.path.exists(goal_path) == False:
        ...
        pass
    return goal_path'''
#修改后函数只改变三个参数，其他参数都固定
def mkdir_if_not_exist(rep, baseline_jce,  decay_jce):
    root_path = os.getcwd()
    
    repeat = '/rep=%03d'%rep
    baseline_jce_files_name = '/baseline_jce=%010f'%baseline_jce
    #is_heterogeneity_files_name = '/is_heterogeneity=%s'%str(is_heterogeneity)
    decay_jce_files_name = '/decay_jce=%010d'%decay_jce
    #patch_dist_rate_files_name = '/patch_dist_rate=%f'%patch_dist_rate
    
    goal_path = root_path + repeat + baseline_jce_files_name + decay_jce_files_name 
    #goal_path =  repeat + baseline_jce_files_name + decay_jce_files_name  
    if os.path.exists(goal_path) == False:
        os.makedirs(goal_path)
    else:
        pass
...
def main(rep, baseline_jce ,decay_jce, goal_path=None):
    ...
    mainland, log_info = generate_empty_mainland(meta_name='mainland', patch_num=1, patch_location_ls=[(0,0)], hab_num=100, hab_length=20, hab_width=20, dormancy_pool_max_size=0, 
                                                 ...
                                                 environment_types_num=environment_types_num, environment_types_name=environment_types_name, environment_variation_ls=environment_variation_ls,baseline_jce =baseline_jce,decay_jce = decay_jce)
    ...
    meta_obj, log_info = generating_empty_metacommunity('metacommunity', patch_num, patch_location_ls, hab_num_in_patch, hab_length, hab_width, int(new_round(total_dormancy_pool_max_size/(patch_num*hab_num_in_patch))), 
                                                        x_axis_environment_values_ls, y_axis_environment_values_ls, environment_types_num, environment_types_name, environment_variation_ls, baseline_jce,decay_jce,is_heterogeneity) 
    write_in_logger_info += log_info
...
if __name__ == '__main__':
    main(rep=0, baseline_jce = 0.5 ,decay_jce = 5)